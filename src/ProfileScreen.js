...
class ProfileScreen extends Component {
  constructor(props) {
    ...
    this.isOwner = !this.isUserProfile
      || (this.userProfile?.profileId && this.userProfile.profileId === props?.userDetails?.profileId);
        ...
        .then(async (currentUser) => {
          if (currentUser) {
            ...
            if (selector === 'posts' || selector === 'likedPosts') {
              ...
              const profileId = this.isUserProfile ? this.userProfile.profileId : userDetails.profileId;
              ...
              debugAppLogger({
                ...
                length: (Array.isArray(results) && results.length) || 'Not an array',
                results,
              });
              ...
              if (Array.isArray(results) && results.length) {
                ...
                if (this.isUserProfile) {
                  this.setState({ [selector]: Array.isArray(items) ? items : [] });
                } else if (selector === 'posts') {
                  this.dispatch(updateUserPosts(Array.isArray(items) ? items : []));
                } else {
                  this.dispatch(updateLikedPosts(Array.isArray(items) ? items : []));
                }
  ...
  render() {
    ...
    const backgroundImage = coverPhotoUrl ? { uri: coverPhotoUrl } : imagePlaceholder;
        ...
        {(headerHeight) => (
          <>
            <StickyParallaxHeader
              ...
              tabTextContainerStyle={{ width: windowWidth / (this.isVendor ? 4 : 3) }}
              tabTextContainerActiveStyle={{ borderBottomWidth: 3, borderBottomColor: '#00D8C6' }}
              // tabsContainerBackgroundColor={colors.primaryGreen}
              tabsWrapperStyle={{ paddingHorizontal: 0 }}
              tabTextStyle={{ fontSize: 16, color: '#777777', paddingVertical: 8 }}
              tabTextActiveStyle={{ fontSize: 16, color: 'black', paddingVertical: 8 }}
              tabsContainerStyle={{ width: windowWidth, flexDirection: 'row', backgroundColor: 'white', paddingHorizontal: 0 }}
              title=""
              tabs={
                this.isVendor
                  ? [
